...
function dependencies {
	...
	[ -z "$deps" ] && return 0
	echo "Puuttuvat apuohjelmat: $deps"
	exit -2
}
...
--overWrite &>/dev/null
	...
	# poista lähtötiedostot ja siirrä tulos haluttuun tai ohjelmakohtaiseen hakemistoon
...
#######
# SAATAVILLA OLEVIEN OHJELMIEN JA JAKSOJEN HAKU
...
function query-programme-episodes {
	 ...
	 *) echo "*** virheellinen ohjelmalähde: $source ***" >&2; exit -4 ;;
	 
	# suodatetaan pois useaan kertaan esiintyvät linkit
	esac | awk '!x[$0]++'
}
...
function katsomo-worker {
	...
	# hae jakson nimi sekä kauden ja jakson numero suoraan www.katsomo.fi-html-sivun kautta
	metadata="$( curl -s -A "${OSX_agent}" "${link/m.katsomo.fi\//www.katsomo.fi/}" |iconv -f ISO-8859-1 )"
	read episode <<<"$( sed -n '/<span class="hidden subtitle-hidden">/ {;n;s#^[ \t]*##;p;}' <<<"$metadata" )"
	IFS=% read snno epno <<<"$( sed -n '/<div class="season-info" style="display:none;">/ {;n;s#.*Kausi \([0-9]*\), jakso: \([0-9]*\).*#\1%\2#p;}' <<<"$metadata" )"

	# hae muut metatiedot /sumo/sl/playback.do-xml-sivun kautta
...
function unified-worker {
	...
	case "$( sed 's#http://\([^/]*\).*#\1#' <<<"$link" )" in
	 ...
	 tv5.fi) tv5-worker "$@" ;;
	 *) echo "*** VIRHEELLINEN LINKKI: \"${link}\" ***" >&2; exit -1
	esac
}


###########
# REGEX-TALLENNUSRUTIINI
...
if [ $# -eq 0 ]
 then if [ -n "$( echo "${vhs}"/*.vhs )" ]
 	 then recording-worker; exit 0
	 else echo "Ei asetettuja tallentimia!"; echo
	fi
fi
